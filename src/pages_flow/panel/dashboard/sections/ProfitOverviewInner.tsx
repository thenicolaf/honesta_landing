...
function aggregate(
  ...
  for (const row of data.rows) {
    ...
    if (row.mix_composition && row.mix_composition.length > 0) {
      for (const entry of row.mix_composition) {
        ...
        const revenueShare = entry.price * entry.count * row.quantity;
        cogs += costShare;
  ...
  return { revenue, cogs, profit, margin, topProducts };
}
  ...
  return (
    <>
      <div className="flex flex-col gap-3 mb-4 sm:flex-row sm:items-center sm:justify-between">
        <SectionHeading className="mb-0">Profit</SectionHeading>
        <DateRangeSelector />
      </div>
      ...
      <SectionHeading>Top profit products</SectionHeading>
      {stats.topProducts.length === 0 ? (
        <EmptyState
          icon={<TrendingUp className="w-10 h-10 text-earth/15" />}
          label="No profit data yet"
